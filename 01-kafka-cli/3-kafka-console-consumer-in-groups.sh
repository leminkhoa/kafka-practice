...
kafka-topics.sh --command-config playground.config --bootstrap-server cluster.playground.cdkt.io:9093 --topic third_topic --create --partitions 3
...
# create a topic with 3 partitions
kafka-topics.sh --bootstrap-server localhost:9093 --topic third_topic --create --partitions 3
...
# start another consumer part of a different group from beginning
kafka-console-consumer.sh --bootstrap-server localhost:9093 --topic third_topic --group my-second-application --from-beginning